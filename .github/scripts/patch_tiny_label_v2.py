...
if start == -1 or end == -1:
    raise SystemExit('Meeting timeline section markers not found')
...
# Lightweight static guardrails for the intended refactor.
result = path.read_text(encoding='utf-8')
required = [
    'MeetingTinyLabelsOverlay',
    'keepPlacementsInsideTimeline',
    'MeetingTinyLabelSide',
]
for marker in required:
    if marker not in result:
        raise SystemExit(f'missing marker after patch: {marker}')
if 'tinyLane:' in result:
    raise SystemExit('old tinyLane layout still present')
