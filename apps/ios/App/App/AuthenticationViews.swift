...
struct SukavinaAppView: View {
    ...
    // Keep the root View associated type small. Xcode 26 emitted a very large
    // conditional SwiftUI metadata graph here; iOS 17.4 aborts while resolving
    // that graph during the first layout pass, before the app can run code.
    var body: AnyView {
        AnyView(
            ZStack {
                primaryContent
    ...
    private var primaryContent: AnyView {
        switch session.state {
        case .restoring:
            AnyView(NativeLaunchView())
        case .signedOut:
            AnyView(AuthenticationView().environmentObject(session))
        case .signedIn:
            if session.profile?.accountType == "CANTEEN" && session.profile?.employeeCode != "DEMO" {
                AnyView(CanteenScannerView().environmentObject(session))
            } else {
                AnyView(EmployeePortalView().environmentObject(session))
            }
        }
    }
}
